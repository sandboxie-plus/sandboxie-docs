...
打开您希望增强功能的沙盒的 **[Sandboxie Ini](../Content/SandboxieIni.md)**。要为该沙盒启用内存盘，请在相应沙盒的部分中添加以下行：
```ini
    UseRamDisk=y
```
### 配置全局设置：

要在所有沙盒中启用内存盘支持，请导航至 **[Sandboxie Ini](../Content/SandboxieIni.md)** 中的 [GlobalSettings] 部分。通过添加以下行来为内存盘分配适当的内存：
```ini
