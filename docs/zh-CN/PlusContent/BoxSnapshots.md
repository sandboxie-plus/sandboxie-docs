...
**快照**可以保存沙盒的当前状态。你可以在不同时刻为一个沙盒创建多个快照，并将其中一个快照设为默认快照。要开始使用，请打开沙盒管理器界面，在目标沙盒上点击右键，并从下拉菜单中选择“快照管理器”。参见下图。
...
**注意事项：**快照只能在禁用沙盒自动删除的情况下创建。要进行设置，请打开沙盒管理器界面，双击目标沙盒，弹出沙盒选项窗口。接着，点击“文件选项”，在“沙盒删除选项”下，取消勾选“自动删除内容”，然后点击右下角的“确定”应用更改。参见下图。
