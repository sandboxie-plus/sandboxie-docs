...
例如，变量 %USERNAME% 会被扩展（或替换）为用户账户名。如果 Sandboxie 无法确定用户账户名（请参阅消息 [SBIE1408](SBIE1408.md) 和 [SBIE2209](SBIE2209.md)），则会发出消息 SBIE1406，并指明变量 USERNAME。

有关可扩展变量的列表，请参阅 [可扩展变量](ExpandableVariables.md)。

**模板变量**

如果消息中的变量名以 **Tmpl** 开头，则应转到 [沙盒设置 > 应用程序 > 文件夹](ApplicationsSettings.md#文件夹) 并选择一个文件夹位置，以关联缺失的变量。
