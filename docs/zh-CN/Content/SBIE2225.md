...
当 Sandboxie 检测到沙盒内的进程尝试访问由 Windows 加密文件系统（EFS）保护的文件且该尝试被阻止或无法代理时，会记录此消息。

该消息包含被尝试访问的文件路径。多数情况下，这表明存在某些配置或证书问题，导致沙盒进程无法获得 EFS 访问权限。

## 常见原因

- 沙盒设置 `EnableEFS` 未在沙盒中启用
...
- 如果已配置 `EnableEFS`，且支持者证书包含加密功能，Sandboxie 会尝试通过将请求代理到用户服务（UserServer）服务，并将句柄复制回沙盒进程的方式完成文件操作
- 如果代理失败，或不允许 `EnableEFS`，Sandboxie 会记录 SBIE2225 并拒绝沙盒进程访问该文件

## 建议的操作

1. 确认沙盒已在 [Sandboxie Ini](SandboxieIni.md) 的沙盒部分或通过沙盒设置界面将 `EnableEFS=y` 配置启用 [^1]
...
- [启用 EFS](EnableEFS.md) — 包含详细说明、示例及本页多处重复引用的附注
- [跟踪日志（适用于 Sandboxie Plus）](SandboxieTrace.md) — 显示 Sandboxie 监管下运行的程序访问的所有系统资源名称

## 实现说明与附注

下列说明引用了 Sandboxie 源代码的实现细节，适用于需要追溯 SBIE2225 消息生成方式的维护者和高级用户。

[^1]: `EnableEFS` 是 [EnableEFS](EnableEFS.md) 中介绍的沙盒级设置。启用后，Sandboxie 允许为沙盒进程代理 EFS 访问请求
