...
[Force Disable Seconds](ForceDisableSeconds.md) controls the pause duration. The menu prompt initially offers the configured value, or 60 seconds when the setting is absent; toolbar and hotkey actions use the configured value or the driver's 10-second fallback. A configured global `ForceDisableSeconds=0` makes the timed pause ineffective; zero does not mean an indefinite pause or permanently disable forcing. The API only saves a supplied duration when it is positive, so entering zero in the menu does not necessarily write a zero-valued INI setting. The pause state remains temporary even if a positive duration is saved as configuration.
