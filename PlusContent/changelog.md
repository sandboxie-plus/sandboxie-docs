...
uncomment: - added optional alternative IPC naming scheme
...
todo: add remember my choice to the sandbox picker
todo: add supprot for silent mode / no notification when gaming
...
-- Note: When sandboxie is running it disables the use of CHPE Binaries for x86 processes globally,
			this is required for the forced processes functionality, this behavioure can be disabled by 
			adding the global option "DisableCHPE=n" to the sandboxie.ini, then x86 processes 
			started outside the sandbox instead of being forced will be terminated with Message SBIE2338
